...
class Evento_Perfil extends Hueso {
  ...
  constructor(_tipo, _padre) {
    super(document.createElement("div"), "perfil perfil_" + _tipo, _padre);
...
class Evento_Aportes extends Hueso {
  // ---------------------------------------------------------------CONSTRUCTOR
  constructor(_padre) {
    super(document.createElement("div"), "aportes", _padre);

    // Espacio para aportes
    this.aportes = [];
    this.innerDiv = new Hueso(
    ...
    this.agregarDiv = new Hueso(
      document.createElement("div"),
      "aportesInner2",
      this,
    );
    this.agregarButt = new Evento_Agregar("aporte", this.agregarDiv);
    this.element.addEventListener("agregar", this.nuevoAporte.bind(this, true));
  ...
  nuevoAporte(_startOpen) {
    let a = new Hueso(
      document.createElement("div"),
      "aporte noContainer textBox",
      this.innerDiv,
    );

    let t = new HuesoTexto("Dato relevante", "aporte_texto", a, _startOpen);

    let iDiv = new Hueso(document.createElement("div"), "icon_container", a);
...
class Evento_Tags extends Hueso {
  // ---------------------------------------------------------------CONSTRUCTOR
  constructor(_tipo, _padre) {
    super(document.createElement("div"), "tags tags_" + _tipo, _padre);

    this.tipo = _tipo;

    // Div para acumular las tags
    this.tags = [];
    this.innerDiv = new Hueso(document.createElement("div"), "tagsInner", this);
    ...
    this.element.addEventListener("agregar", this.nuevaTag.bind(this));
...
class Evento_Agregar extends HuesoFlotante {
  ...
  constructor(_tipo, _padre) {
    ...
    this.icon = new Hueso(document.createElement("div"), "agregar_plus", this);
    this.icon.element.textContent = "+";
    this.getTam();

    // Hace sus cosas al hacerle click
    this.element.addEventListener("click", this.clickSensor.bind(this));
